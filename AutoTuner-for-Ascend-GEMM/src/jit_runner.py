...
"""
CATLASS JIT Runner
昇腾算子即时编译与执行引擎。它充当了调优器与底层硬件之间的桥梁。

主要功能:
1. 模板动力学: 接收多维 Tile 参数，通过正则或字符串替换注入 C++ 源代码模板。
2. 自动化构建: 封装 CMake 与 shell 脚本调用，驱动昇腾后端工具链生成高性能二进制.
3. 性能提取器: 执行下板 binary，并解析其控制台输出以获取 TFLOPS 和耗时。
"""
...
class CATLASSJITRunner:
    ...
    def __init__(self, catlass_dir: Optional[Path] = None):
        ...
        self.template_file = self.base_dir / "src" / "custom_kernel" / "tunable_gemm_template.cpp"
