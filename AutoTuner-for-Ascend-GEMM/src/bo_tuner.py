...
使用GPyOpt内置约束机制，从海量分块方案中自动搜寻最优解，避免无效编译污染GP模型。

核心逻辑：
1. 参数空间定义 (Search Space): 包含 Tile M/N/K, ILP Flags, Swizzle 等。
2. 硬约束过滤 (Hard Constraints): 在下发 JIT 编译前，通过逻辑公式剔除会导致 SRAM 溢出或非法对齐的配置。
3. 贝叶斯迭代: 利用 Gaussian Process 建模，通过 Expected Improvement (EI) 指标推荐下一次采样点。
...
class JITBOTunerV2:
    ...
    def _objective(self, x: np.ndarray) -> float:
        ...
        if gflops == 0 or exec_time > 1e9:
            ...
            # 理由: 如果返回一个极大的惩罚值（如 1e10），高斯过程会认为该区域极度危险，从而彻底放弃附近的探索。
            # 而编译失败通常只代表该点非法，其附近的点可能仍然具有极高的性能潜力。
            gflops = 0.0
