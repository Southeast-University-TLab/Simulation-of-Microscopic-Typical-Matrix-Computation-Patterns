...
本项目围绕昇腾（Ascend）NPU 上矩阵乘法（GEMM）算子的性能建模与自动优化展开研究，旨在通过硬件仿真建模与贝叶斯优化搜索相结合的方式，加速矩阵计算参数搜索并提升算子执行效率。
...
+ 1\.SimNPU
&nbsp;基于 NPU 微观硬件结构的矩阵计算仿真器，对计算核心、片上缓存层级以及数据传输路径进行建模，用于预测矩阵运算的理论性能上限与执行延迟。



+ 2\.AutoTuner-for-Ascend-GEMM
&nbsp;基于贝叶斯优化（Bayesian Optimization）的自动化算子调优框架，通过 \*\*JIT 编译 + 实机运行测试\*\* 的方式，在真实昇腾硬件上搜索高性能 GEMM 算子配置。
...
+ SimNPU：建立硬件级性能模型，预测计算性能上限（Roofline）。
